...
hdr "3. Koneksi"
if [[ -f "$CFG" ]]; then
  if python3 "$SKILL_DIR/scripts/lib/mailbox.py" test >/tmp/hermes-email-doctor.log 2>&1; then
    ok "IMAP login OK"
    ok "SMTP login OK"
...
for f in profile.py ledger.py handoff.py; do
  [[ -f "$SKILL_ROOT/scripts/lib/$f" ]] && ok "scripts/lib/$f ada" \
    || p_fail "scripts/lib/$f hilang — jalankan: bash shared/sync.sh"
done
...
if [[ -f "$SKILL_ROOT/scripts/lib/handoff.py" ]]; then
  QN="$(python3 "$SKILL_ROOT/scripts/lib/handoff.py" list 2>/dev/null | grep -cE '^  \[[0-9]+\]' || true)"
  if [[ "${QN:-0}" -gt 0 ]]; then
    p_warn "$QN chat nunggu jawaban pemiliknya — bersihkan: python3 scripts/lib/handoff.py list"
  else
    ok "Antrean handoff kosong"
  fi
fi
python3 "$SKILL_ROOT/scripts/lib/profile.py" check >/dev/null 2>&1 \
