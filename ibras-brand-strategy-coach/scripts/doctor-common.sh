...
# terbaca? guard terpasang?), dan `ibras-brand-strategy-coach` — pintu masuk utama —
...
doctor_common() {
  ...
  check "profil terbaca"                python3 "$here/scripts/lib/profile.py" check
  ...
  # Hermes Hub menulis file unduhan tanpa mempertahankan executable bit.
  # Semua Python dipanggil lewat python3, jadi keberadaan file adalah kontraknya.
  check "check-numbers.py tersedia"   test -f "$here/scripts/check-numbers.py"
  check "check-citations.py tersedia" test -f "$here/scripts/check-citations.py"
  check "artifact-guard.py tersedia"  test -f "$here/scripts/hooks/artifact-guard.py"
  warn_check "guard terpasang di ~/.hermes"  bash "$here/scripts/install-guard.sh" --check

  echo ""
  echo "Rem:"
  if python3 "$here/scripts/lib/halt.py" status >/dev/null 2>&1; then
...
doctor_summary() {
  ...
  echo "Perbaikan paling sering: pasang ulang atau perbarui skill dari sumber distribusi." >&2
  return 1
}
