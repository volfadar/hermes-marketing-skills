...
if [[ "$STAGE" == "stage3" && -n "$DATA" ]]; then
  ...
  STMT="$(printf '%s' "$DATA" | python3 -c "import sys,json; d=json.load(sys.stdin); d=d.get('positioning',d); c=d.get('chosen',{}); print(d.get('statement') or (c.get('public_line','') if isinstance(c,dict) else ''))" 2>/dev/null)"
  echo "  ${STMT:-}"
  echo ""
  echo "Lalu set pillars di ibras-content-creator:"
  echo "  bash ~/.hermes/skills/ibras-content-creator/scripts/pillars.sh \"<pilar1>, <pilar2>, <pilar3>\""
fi
