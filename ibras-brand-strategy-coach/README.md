...
```bash
cp -R ibras-brand-strategy-coach ~/.hermes/skills/ibras-brand-strategy-coach
```
...
bash ~/.hermes/skills/ibras-brand-strategy-coach/scripts/start-session.sh --user "$USER_NAME"

# (jalankan sebagai percakapan bertahap, lalu simpan evidence yang sudah dikonfirmasi)
bash ~/.hermes/skills/ibras-brand-strategy-coach/scripts/save-profile.sh stage1 \
  --data '{"talent":{"tests":{},"hypotheses":[],"behavioral_evidence":[],"accommodations":[]}}'

# Stage 2: Background
bash ~/.hermes/skills/ibras-brand-strategy-coach/scripts/stage2.sh --user "$USER_NAME"
# ... dst sampai stage 5
```
