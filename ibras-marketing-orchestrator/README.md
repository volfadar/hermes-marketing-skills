...
```bash
cp -R ibras-marketing-orchestrator ~/.hermes/skills/ibras-marketing-orchestrator
```
...
`ibras-brand-strategy-coach/references/`, jadi lima dari enam skill tidak bisa melihatnya.
...
Semua yang di `scripts/lib/`, `references/`, `scripts/`, dan `scripts/hooks/` disalin dari `shared/`
