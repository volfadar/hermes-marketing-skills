...
cat <<'TXT'
marketing-orchestrator — memutuskan skill mana yang jalan, dan urutannya
...
RUTE
  belum ada harga di `fakta`        -> brand-strategy-coach Stage 1 (90 detik)
  ada produk, copy-nya generik      -> brand-strategy-coach Stage 2b saja (~15 mnt)
  benar-benar memilih arah          -> positioning lab penuh (sebut kalau ini yang dipakai)
  butuh bukti dari luar             -> cloakserve-research (halaman publik saja)
  komentar/DM/jadwal IG·FB·TikTok·YT·Threads -> Repliz (dibeli, bukan dibangun)
  WhatsApp                          -> waha-marketing (Repliz tidak menyentuh WA)
  email                             -> email-marketing (Repliz tidak menyentuh email)
  mau jalan sendiri tiap pagi       -> python3 lib/watch.py create
  datang lagi besok pagi            -> python3 lib/ledger.py open
...
MENUTUP SESI — selalu sama
  python3 lib/ledger.py show     apa yang jam ini hasilkan
