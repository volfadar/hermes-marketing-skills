...
for f in options.yaml platforms.yaml sources.yaml; do
  if [[ -f "$SKILL_DIR/assets/data/$f" ]]; then
    if python3 -c "import yaml,sys; yaml.safe_load(open('$SKILL_DIR/assets/data/$f',encoding='utf-8'))" 2>/dev/null; then
      ok "assets/data/$f valid"
    else
      p_fail "assets/data/$f ada tapi YAML-nya rusak"
    fi
  else
    p_fail "assets/data/$f hilang"
  fi
done
...
opts = yaml.safe_load((root/"assets/data/options.yaml").read_text(encoding="utf-8"))
srcs = yaml.safe_load((root/"assets/data/sources.yaml").read_text(encoding="utf-8"))
plats = yaml.safe_load((root/"assets/data/platforms.yaml").read_text(encoding="utf-8"))
...
if python3 "$SKILL_DIR/scripts/lib/advisor.py" options >/dev/null 2>&1; then
  ok "advisor.py options jalan"
else
  p_fail "advisor.py options gagal"
fi
if python3 "$SKILL_DIR/scripts/lib/advisor.py" recommend --budget 5 >/dev/null 2>&1; then
