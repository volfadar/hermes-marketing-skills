...
metadata:
  hermes:
    ...
    related_skills: [cloakserve-research]
---
...
python3 lib/profile.py show     # harga & suara dia — biar nggak nanya ulang
```
...
di `lib/broadcast.py` ada untuk menjaga nomor itu tetap hidup.
...
```bash
python3 lib/copycheck.py templates.txt        # promo
python3 lib/copycheck.py --text "..." --reply # balasan ke satu orang
```
...
- Python 3 (untuk `lib/broadcast.py`)
...
`lib/broadcast.py` implements ALL the WAHA-official anti-ban safeguards:
...
4. **"Balesin otomatis aja" — ini permintaan yang sah, jangan ditembok.**
   ...
   **Yang naik ke dia harus punya tempat mendarat**, kalau tidak "selalu naik
   ...
   ```bash
   python3 lib/handoff.py list                    # siapa yang nunggu
   ...
   berbahaya. `python3 lib/handoff.py stats` menunjukkan mana yang paling
