...
#
# scripts/lib/broadcast.py already paces properly — 12-45 s between messages, long
...
" 2>/dev/null || get "/api/messages?chatId=$CID_ENC&session=$WAHA_SESSION&limit=$LIM" | pp | head -30
  ...
  send-text)
    ...
    if [[ "$CONFIRM" != "yes" ]]; then
      ...
      # avoid.
      CHECK_RC=0
      CHECK_OUT="$(python3 "$SKILL_DIR/scripts/lib/outbound_checks.py" --text "$TXT" \
      ...
      if [[ $CHECK_RC -eq 3 ]] && python3 - "$TXT" "$SKILL_DIR/scripts/lib" <<'PY' 2>/dev/null
import sys
...
MSG
      ...
      python3 "$SKILL_DIR/scripts/lib/ledger.py" add --kind sent --who "${CID%%@*}" \
  ...
  recap)
    python3 "$SKILL_DIR/scripts/lib/ledger.py" show
    ;;
  open)
    python3 "$SKILL_DIR/scripts/lib/ledger.py" open
    ;;
  week)
    python3 "$SKILL_DIR/scripts/lib/ledger.py" week
    ;;
  profile)
    python3 "$SKILL_DIR/scripts/lib/profile.py" "${1:-show}"
    ;;
