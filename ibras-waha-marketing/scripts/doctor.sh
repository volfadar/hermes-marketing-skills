...
command -v python3 >/dev/null && ok "python3 available" || p_fail "python3 tidak ada (dibutuhkan scripts/lib/broadcast.py)"
...
for f in profile.py copycheck.py ledger.py handoff.py; do
  if [[ -f "$SKILL_ROOT/scripts/lib/$f" ]]; then ok "scripts/lib/$f ada"
  else p_fail "scripts/lib/$f hilang — jalankan: bash shared/sync.sh"; fi
done
...
if PROF_OUT="$(python3 "$SKILL_ROOT/scripts/lib/profile.py" check 2>&1)"; then
  if python3 "$SKILL_ROOT/scripts/lib/profile.py" get sikap.kenapa_aku >/dev/null 2>&1; then
    ok "Profil usaha terisi ($(echo "$PROF_OUT" | tr -d '\n' | sed 's/^ *//'))"
  else
    p_warn "Profil belum punya 'sikap' — promo bakal generik. Isi lewat ibras-brand-strategy-coach."
  fi
else
