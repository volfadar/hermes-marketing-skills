...
[[ -d "$SKILLS/cloakserve-research" ]] && ok "cloakserve-research available (for trend research)" || p_warn "cloakserve-research tidak terinstall (opsional, untuk riset tren)"
[[ -d "$SKILLS/waha-marketing" ]] && ok "waha-marketing available (for WA list building)" || p_warn "waha-marketing tidak terinstall (opsional)"
