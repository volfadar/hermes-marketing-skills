...
metadata:
  hermes:
    ...
    related_skills: [cloakserve-research, waha-marketing]
---
...
with their real costs and drawbacks, live in `skill-social-publishing`. Send
...
- "Ini voice note dari pelanggan, tolong ringkas." → transkrip, ambil intinya,
  satu baris ke `lib/ledger.py add --kind waiting`. **Voice note 3 menit yang tidak
...
- **"Auto-post tiap jam 9 pagi"** — bisa, dan jalur resminya aman. Tunjukkan
  Meta Business Suite / Buffer lewat `skill-social-publishing`, sarankan
...
- (Optional) cloakserve-research skill — for trend/topic research.
...
2. **For ideation:** use `ideate.sh` which pulls from pillars + recent trends
   (via cloakserve-research if available). Always offer 5+ options, never 1.
...
lagi — `skill-social-publishing` punya enam jalur lengkap dengan kerugian
