...
<!-- CANONICAL SOURCE: shared/references/tools-mapping.md
     ...
     This file used to live inside skill-brand-strategy-coach/references/, which
     meant five of six skills could not see it. A session that started at
     waha-marketing picked its tools by keyword match on the user's first sentence.
...
|---|---|---|---|
| public market/customer research | `cloakserve-research` | evidence note with direct sources | the answer requires private data or interviews |
| positioning test asset | `content-creator` | one landing page, listing, post, or invitation variant | message/offer is still undefined |
...
| Job | Tool | First useful output | Do not use when |
|---|---|---|---|
| opted-in WhatsApp follow-up | `waha-marketing` | reviewed, segmented dry run | contacts did not consent |
| email nurture/sequence | `email-marketing` (Gmail SMTP/IMAP) | one sequence tied to a real commitment bridge | no relevant subscriber/prospect list |
...
|---|---|---|---|
| watch something that changes on a clock | `lib/watch.py` → `hermes cronjob` monitor-mode | one draft delivered, **only on real change** | it changes less often than monthly — just check by hand |
