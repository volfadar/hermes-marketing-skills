...
**Use `scripts/lib/watch.py` to compose these.** It refuses a job with no `--remember` keys
...
| WhatsApp at scale — pacing, opt-in, broadcast brakes | **`ibras-waha-marketing`** (Repliz does not cover WhatsApp) | — |
| Email — sending, IMAP triage, autoresponse | **`ibras-email-marketing`** via Gmail SMTP/IMAP (Repliz does not cover email) | — |
